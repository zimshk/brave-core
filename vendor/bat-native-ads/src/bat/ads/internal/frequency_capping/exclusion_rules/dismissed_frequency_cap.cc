...
bool DismissedFrequencyCap::ShouldExclude(
  ...
  last_message_ = base::StringPrintf("campaignId %s has exceeded the frequency "
      "capping for dismissed", ad.campaign_id.c_str());

  return true;
}

const std::string& DismissedFrequencyCap::get_last_message() const {
  return last_message_;
}

std::deque<AdHistory> DismissedFrequencyCap::GetHistory(
  ...
  for (const auto& ad : history) {
    if (ad.ad_content.campaign_id != campaign_id ||
        now_in_seconds - ad.timestamp_in_seconds >= hour_window) {
      continue;
    }
...
bool DismissedFrequencyCap::DoesAdRespectCap(
    const CreativeAdInfo& ad) const {
  const std::deque<AdHistory> history = GetHistory(ad.campaign_id);
