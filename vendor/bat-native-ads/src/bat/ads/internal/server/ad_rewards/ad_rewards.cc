...
double AdRewards::GetEstimatedPendingRewards() const {
  const double estimated_pending_rewards = CalculateEstimatedPendingRewards();
  ...
  return estimated_pending_rewards + unredeemed_estimated_pending_rewards
      + unreconciled_estimated_pending_rewards_;
}

uint64_t AdRewards::GetNextPaymentDateInSeconds() const {
  const base::Time now = base::Time::Now();
  const base::Time next_payment_date = payments_->CalculateNextPaymentDate(
      now, ads_->GetNextTokenRedemptionDateInSeconds());
...
uint64_t AdRewards::GetAdNotificationsReceivedThisMonth() const {
  const TransactionList transactions =
      ads_->get_confirmations()->GetTransactions();
