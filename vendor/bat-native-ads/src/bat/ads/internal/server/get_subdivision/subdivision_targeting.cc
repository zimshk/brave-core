...
#include "bat/ads/internal/locale/subdivision_codes.h"
...
bool SubdivisionTargeting::ShouldAllowForLocale(
  ...
  const SubdivisionCodesSet subdivision_codes =
      kSubdivisionCodes.at(country_code);
...
bool SubdivisionTargeting::IsSupportedLocale(
  ...
  const auto iter = kSubdivisionCodes.find(country_code);
  if (iter == kSubdivisionCodes.end()) {
    return false;
  }
