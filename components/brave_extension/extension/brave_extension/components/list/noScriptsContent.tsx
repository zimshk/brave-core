...
interface Props {
  noScriptActions: any//typeof noScriptActions
...
export default class NoScriptList extends React.PureComponent<Props, {}> {
  get actions () {
    return this.props.noScriptActions
  }
  ...
  setBlockState (url: string) {
    this.actions.setScriptBlockedCurrentState(url)
  }

  setBlockStateGroup (hostname: string, maybeBlock: boolean) {
    this.actions.setGroupedScriptsBlockedCurrentState(hostname, maybeBlock)
  }
  ...
  getSingleScriptRow = (url: string, scriptData: NoScriptEntry, key: number, shouldBlock: boolean) => {
    return (
      <BlockedListItemWithOptions key={key}>
        ...
        <LinkAction onClick={this.setBlockState.bind(this, url)}>
