...
std::pair<int64_t, uint64_t> GetAutoContributionCountOnFileTaskRunner(
  ...
  uint64_t last_reconcile_date = backend->GetLastReconcileDate();
  return std::make_pair(count, last_reconcile_date);
}
...
void RewardsServiceImpl::OnReconcileComplete(ledger::Result result,
  ...
  BroadcastReconcile(contribution_count_ != 0);
  if (result == ledger::Result::LEDGER_OK) {
    auto now = base::Time::Now();
    if (!Connected())
      return;
...
void RewardsServiceImpl::BroadcastReconcile(
    bool contribution_in_progress) {
  for (auto& observer_ : observers_) {
    observer_.OnContributionInProgressChanged(this, contribution_in_progress);
  }
}
